function RegisterTextboxInstance(p_instance)
{
	...
	assert(global.g_textboxInstance == noone, "textboxInstance variable should be initialized by the object_global to 0 first. Make sure that object_global is initialized first in the room order");
	// Check if the parameter instance is valid
	assert(p_instance != 0, "Passed textbox isn't valid");
